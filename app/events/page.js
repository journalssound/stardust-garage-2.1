...
export default async function EventsPage() {
  ...
  const [headerImages, eventsResult] = await Promise.all([
    supabase.from('gallery_images').select('*').eq('gallery_key', 'events_header').order('sort_order', { ascending: true }),
    supabase.from('events').select('*').order('event_date', { ascending: true }),
  ]);

  const images = headerImages.data || [];
